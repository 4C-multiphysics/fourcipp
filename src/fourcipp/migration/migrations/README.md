...
Each file is named `<version>.yaml`, where `<version>` is the input file version that its
migrations upgrade *to*, written as a number zero-padded to 5 digits, e.g. `00002.yaml`
contains the changes needed to go from the previous known version to version `00002`. The
padding keeps numeric and lexicographic ordering identical, so the files sort correctly in
any tool, whether it sorts them as text or as numbers. The width is defined by
`VERSION_DIGITS` in `fourcipp/migration/database.py`. Every file contains a single top-level
key:
...
All paths (`path`, `old_path`, `new_path`) are lists of string keys, walking
